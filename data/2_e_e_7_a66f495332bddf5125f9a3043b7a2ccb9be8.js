  ...
  */
  ...
  reconnect: function() {
    ...
    if(dpManager.isCollaboration()) {
      socket.emit('addCollaborator', {'socketRoomId': dpManager.dpProject.getHash(), 'userId': dpManager.DataForCollaboration.user_id});
    };
...
var UXPConnection = new Class({
  ...
  initialize: function() {
    ...
    window.addEventListener('offline', this.setDisconnected.bind(this));
  ...
  setConnected: function(pFireEvent) {
    ...
    var fire_event = undefined === pFireEvent || pFireEvent;
...
var DPPageComments_Add = new Class({
  ...
  saveComment: function() {
    ...
    if(window.mixpanel) {
      mixpanel && mixpanel.track('Add comment');
    }
  },
...
var DPPageComments = new Class({
  ...
  createComment: function(pOptions, pAddChild) {
    ...
    if (!pOptions.id_page_comment_parent) {
      var pin = new DPPageComments_Pin(comment);
    }
    
    this.mListRef.createItem(comment);
...
var DPPageComments_Pin_Popup = new Class({
  ...
  save: function(pMode) {
    ...
    mixpanel && mixpanel.track('Add comment response');
  },
