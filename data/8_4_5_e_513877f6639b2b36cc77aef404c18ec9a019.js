...
;(function Preview_ElementScope() {
  ...
  var Preview_Elements = new Class({
    ...
    set: function(pId, pEl, pOptions) {
      var el = typeof pEl == 'object' ? pEl : Interactions.getSetElRef(pId);

      if (!el) {
        console.error('NO ELEMENT ' + pId);
        return;
      }
      ...
      this.mIndex[pId] = new window[class_name](pEl, pOptions);
      this.mIndex[pId].setId(pId);
    },

    get: function(pId) {
      return this.mIndex[pId];
    },
...
;(function Preview_Elements_Generic_Scope() {
  ...
  var Preview_Elements_Generic = new Class({
    ...
    setInteractive: function() {
      this.mEl.classList.add('interactive');
      this.mEl.classList.add('highlighted');
    },

    getSetElementAnimations: function() {
      if (!this.mElementAnimations || !(this.mElementAnimations instanceof InteractionsAnimations)) {
        this.mElementAnimations = new InteractionsAnimations(this.mEl);
      }
     ...
     * @todo make use of pAnimation parameter - move it from fn_style.tpl
     *
     * @param pStyle
     * @param pValue
     ...
     */
    setStyle: function(pStyle, pValue, pUpdateOriginal) {
      var update_original = typeof pUpdateOriginal == 'undefined' || pUpdateOriginal;
      var style_property = this.cssToJsStyle(pStyle);
      var current_style = window.getComputedStyle(this.mEl)[style_property];

      this.mEl.style[style_property] = pValue;
    ...
    resize: function(pWidth, pHeight, pAnimation) {
      ...
      if (pAnimation && pAnimation.style) {
        this.mElementAnimations.addAnimation('resize', pAnimation.style, pAnimation.time + 'ms');
        ...
        InteractionsAnimations.timeouts.resize[this.mId] = setTimeout(function removeResizeScope() {
          this.mElementAnimations.remove('resize');
      ...
      this.mEl.style.width = (parseInt(pWidth)) + 'px';
      this.mEl.style.height = (parseInt(pHeight)) + 'px';
    }
  ...
  var Preview_Elements_GeoSVG = new Class({
    ...
    setStyle: function(pStyle, pValue, pUpdateOriginal) {
      ...
      if ('color' == pStyle) {
        ...
        this.parent(pStyle, pValue, pUpdateOriginal);
      }
    }
  ...
  var Preview_Elements_IconSVG = new Class({
    ...
    setStyle: function(pStyle, pValue, pUpdateOriginal) {
      ...
      if ('color' == pStyle) {
        ...
        this.parent(pStyle, pValue, update_original);
      }
    }
...
;(function Preview_Elements_Image_Scope() {
  ...
  var Preview_Elements_Image = new Class({
    ...
    resize: function(pWidth, pHeight, pAnimation) {
      if (pAnimation && pAnimation.style) {
        var element_animations = new InteractionsAnimations(this.mElImage);
        element_animations.addAnimation('resize', pAnimation.style, pAnimation.time + 'ms');

        InteractionsAnimations.timeouts.resize[this.mId] = setTimeout(function() {
          element_animations.remove('resize');
        }, InteractionsAnimations.getTimeoutTime(pAnimation.time));
      }
...
;(function Preview_Elements_Navigation_Scope() {
  ...
  var Preview_Elements_Navigation = new Class({
    ...
    setStyle: function(pStyle, pValue, pUpdateOriginal, pAnimate) {
      ...
      if ('color' == pStyle) {
        return this.parent(pStyle, pValue, pUpdateOriginal);
      }

      // set transitions
      if (pAnimate && pAnimate.style) {
        Array.prototype.forEach.call(this.mElements, function(pElement) {
          var animations = new InteractionsAnimations(pElement);
          animations.addAnimation('style_navi', pAnimate.style, (parseInt(pAnimate.time) || 300) + 'ms');

          InteractionsAnimations.timeouts.style[el_id] = setTimeout(function() {
            animations.remove('style_navi');
          }, InteractionsAnimations.getTimeoutTime(pAnimate.time));
        });
      }
...
;(function Preview_Elements_Tab_Scope() {
  ...
  var Preview_Elements_Tab = new Class({
    ...
    setStyle: function(pStyle, pValue, pUpdateOriginal, pAnimate) {
      ...
      // font color set in standard way
      if ('color' == pStyle) {
        return this.parent(pStyle, pValue);
      }
...
;(function Preview_Elements_TextElement_Scope() {
  ...
  var Preview_Elements_TextElement = new Class({
    ...
    initialize: function(pEl, pOptions) {
      this.parent(pEl, pOptions);
      this.mChildElements = this.mEl.querySelectorAll('span[style]') || this.mEl;
    },
    ...
    setStyle: function(pStyle, pValue, pUpdateOriginal, pAnimate) {
      ...
      if (pAnimate && pAnimate.style) {
        Array.prototype.forEach.call(this.mChildElements, function(pElement) {
          var animations = new InteractionsAnimations(pElement);
          animations.addAnimation('style', pAnimate.style, (parseInt(pAnimate.time) || 300) + 'ms');

          InteractionsAnimations.timeouts.style[this.mId] = setTimeout(function() {
            animations.remove('style');
          }, InteractionsAnimations.getTimeoutTime(pAnimate.time));
        }.bind(this));
      }
...
;(function PreviewInteractionsScope(global) {
  ...
  var Interactions = {
    ...
    isVisible: function(el) {
      return window.getComputedStyle(el).getPropertyValue('opacity') > 0 && window.getComputedStyle(el).getPropertyValue('visibility') !== 'hidden';
    },
    ...
    showEl: function(id) {
      var el = this.getSetElRef(id);

      el.style.visibility = 'visible';
      el.style.marginLeft = '0px';
      el.style.opacity = Interactions.ori_styles[id].opacity;
    },

    hideEl: function(id, slideout) {
      var el = this.getSetElRef(id);
    ...
    isSvg: function(el) {
      var svg_elements = ['GeoSVGEl', 'UniSVGEl', 'IconSVG'];

      return svg_elements.some(function(pSvgClass) {
       return el.classList.contains(pSvgClass);
      });
    },
    ...
    handleEvent: function(e) {
      ...
      if (e.keyCode && ['INPUT', 'TEXTAREA', 'SELECT'].indexOf(e.target.nodeName) > -1) {
        return false;
      }
...
;(function InteractionsAnimationsScope(global) {
  ...
  InteractionsAnimations.prototype.parseCurrentAnimations = function() {
    ...
    var animations_callback = function(property) {
      ...
      // index of animation of the property in the transitions arrays
      var index = transitions.property.indexOf(property);
    ...
    for (var type in InteractionsAnimations.definitions) {
      if (InteractionsAnimations.definitions.hasOwnProperty(type)) {
        ...
        InteractionsAnimations.definitions[type].forEach(animations_callback);
      }
    }
  };
  ...
  /**
   * @todo remove only when the animation is set by this instance
   *
   * @param type
   */
